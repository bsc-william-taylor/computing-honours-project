...
function blur(program, context, commandQueue) {
  with(cl) {
    const kernel = clCreateKernel(program, "vector_add", null);
    const img = fs.readImage('image.jpg');
    ...
    const globalWorkSize = Uint32Array.from([120, 75]);
    ...
    clSetKernelArg(kernel, 3, 4, img.height);
    clSetKernelArg(kernel, 4, 4, img.width);

    const output = new ArrayBuffer(img.width * img.height * img.bpp);
    const region = [img.width, img.height, 1];
    const origin = [0, 0, 0];
...
with(cl) {
  ...
  //vector_add(program, context, commandQueue);
  blur(program, context, commandQueue);
